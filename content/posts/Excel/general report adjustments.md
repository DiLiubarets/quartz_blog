...
Sub CombineSheets_withCol()
    ...
    newHeaders = Array("Original Estima hrs", "Remaining hrs", "Time Spent hrs", "Original Estima SP", "Remaining SP", "Time Spent SP", "EV, SP", "WP")

    ' Create or clear the "CombinedData" sheet
    On Error Resume Next
    Set combinedWs = ThisWorkbook.Sheets("CombinedData")
    On Error GoTo 0
    ...
    ' Loop through all sheets and combine data
    For Each ws In ThisWorkbook.Worksheets
        If ws.Name <> "Instructions" And ws.Name <> "CombinedData" Then
            If firstSheet Then
                ws.UsedRange.Copy Destination:=combinedWs.Cells(nextRow, 1)
    ...
    ' "WP" Column: Extract text before " :"
    If Not epicLinkCol Is Nothing And Not wpCol Is Nothing Then
        For i = 2 To lastRow
            combinedWs.Cells(i, wpCol.Column).Formula = "=TEXTBEFORE(" & combinedWs.Cells(i, epicLinkCol.Column).Address(False, False) & ","" :"")"
        Next i
    End If

    ' "Original Estima SP" Column: Divide "Original Estima hrs" by 4
    ...
    MsgBox "All sheets combined and formulas applied successfully!"
